...
class Section1 extends Section {
  ...
  initModel() {
    ...
    this.cameraAction = this.animationMixer.clipAction(this.model.animations.find(a => a.name === 'World_animation'));
    this.cameraAction.play().paused = true;
    this.actionsToAnimate.push(this.cameraAction);

    this.ch1Action = this.animationMixer.clipAction(this.model.animations.find(a => a.name === 'CH_1_action'));
    this.ch2Action = this.animationMixer.clipAction(this.model.animations.find(a => a.name === 'CH_2_action'));
    this.ch3Action = this.animationMixer.clipAction(this.model.animations.find(a => a.name === 'CH_3_action'));

    this.animationMixer.addEventListener('finished', (event) => {
      if (event.action === this.ch1Action || event.action === this.ch2Action || event.action === this.ch3Action) {
  ...
  initDebugGui() {
    ...
    gui.add(props, 'sectionAnimationInertia1', 0, 0.1, 0.000001).onChange(value => this.animationInertia = value);
    gui.add(props, 'sectionAnimationInertiaBack1', 0, 0.1, 0.000001).onChange(value => this.animationInertiaBack = value);
    gui.add(props, 'hue1', 0, Math.PI * 2, 0.0001).onChange(value => this.effects.hueSaturation.hue = value);
    gui.add(props, 'saturation1', -1, 1, 0.0001).onChange(value => this.effects.hueSaturation.saturation = value);
